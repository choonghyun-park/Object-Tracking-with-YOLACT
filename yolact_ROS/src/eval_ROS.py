...
from layers.box_utils import jaccard, center_size, mask_iou, point_form
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Copy'):
        ...
        # -------------------Here is the clases, scores, boxes-------------------------/
    ...
    for m in masks:
        cv2.line(img_numpy, centerOfMass(m),centerOfMass(m),(0,255,0),5)


    # final point of making mask and etc.
    global msg  
    msg = centers_msg(masks, classes, scores)

    return img_numpy

# msg publish using std_msgs
def make_Int64_msg(masks, i):
    msg = Int64()
    msg.data = middle_mask(masks[i])[0]
    return msg

# calulate and make msg using centerOfMass
def centers_msg(masks, classes, scores):
    # print("Detections :",classes)
    msg = Segment_centers()
    for i in range(len(classes)):   
        msg.id.append(classes[i])
        msg.pixel_x.append(centerOfMass(masks[i])[0])
        msg.pixel_y.append(centerOfMass(masks[i])[1])
    return msg

# Calculate the center point using longest list position
def middle_mask(mask):
    mask_sum_0 = mask.sum(dim=0)
    mask_sum_1 = mask.sum(dim=1)

    max_tensor_0 = torch.argmax(mask_sum_0) 
    max_tensor_1 = torch.argmax(mask_sum_1)
    
    max_0 = max_tensor_0.item() # 0-640
    max_1 = max_tensor_1.item() # 0-480

    return (max_0, max_1)

# It calculates the center of Mass of mask
def centerOfMass(mask):
    mask_sum_0 = mask.sum(dim=0)
    mask_sum_1 = mask.sum(dim=1)
    
    centroid_x = 0
    for i, s in enumerate(mask_sum_0):
        centroid_x += i*s
    centroid_x /= mask_sum_0.sum()

    centroid_y = 0
    for i, s in enumerate(mask_sum_1):
        centroid_y += i*s
    centroid_y /= mask_sum_1.sum()
    
    centroid_x = int(centroid_x.item())
    centroid_y = int(centroid_y.item())
...
def evalimage_rs(net:Yolact):
    ...
    try:
        while True:
            ...
            # yolact segmentation detecting
            with torch.no_grad():
                frame = torch.from_numpy(img_color).cuda().float()
            ...
            for i in range(len(msg.pixel_x)):
                if len(msg.x)>=len(msg.id):
                    break
                depth_pixel = [msg.pixel_x[i], msg.pixel_y[i]]
                dis, camera_coordinate = get_3d_camera_coordinate(depth_pixel, aligned_depth_frame, depth_intrin)
                msg.x.append(camera_coordinate[0])
                msg.y.append(camera_coordinate[1])
                msg.z.append(camera_coordinate[2])
                msg.secs.append(rospy.get_time())
            
            pub.publish(msg)
            
            # Show images
            cv2.namedWindow('RealSense', cv2.WINDOW_AUTOSIZE)
            cv2.imshow('RealSense', img_numpy)
            cv2.waitKey(1)
...
def get_aligned_images(pipeline, align):
    ...
    img_depth = np.asanyarray(aligned_depth_frame.get_data())       # 16
...
def evalvideo_ros(data, e_arg):
    # If the path is a digit, parse it as a webcam index
    net = e_arg #net:Yolact
    bridge = CvBridge()
    vid = bridge.imgmsg_to_cv2(data, desired_encoding='bgr8')
    cv2.imshow("image",vid)
    cv2.waitKey(50)
    
    if vid is None:
        print('Could not open video of topic')
        exit(-1)
        ...
        try:
            ...
            while running:
                ...
                if not frame_buffer.empty():
                    ...
                    # show video
                        cv2.imshow('topic', frame_buffer.get())
                        cv2.waitKey(50)
...
def  evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        ...
        elif args.video == "depth":
            rospy.Subscriber("camera/color/image_raw",ImageMsg, evalvideo_ros, net)
            rospy.spin()
...
def yolact_pub():
    pub = rospy.Publisher('yolact_pub', Segmentation2) # commonly file name
    rospy.init_node('yolact_pub', anonymous=True) # topic name
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if not os.path.exists('results') and not args.image=='depth':
            os.makedirs('results')
        ...
        pub = rospy.Publisher('yolact_ros', Segment_centers, queue_size=10) 
